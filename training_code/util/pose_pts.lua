...
function generateSample(set, idx)
    ...
    for i = 1,dataset.nJoints do
	    ...
	    pts_out[i] = transform(pts[i], c, s, r, opt.outputRes)
    ...
    print(s)
    print(r)
    image.save('./test/ori.jpg',img)
    image.save('./test/crop.jpg',inp)
    matio.save('./test/crop.mat',pts_out:float())
    im_show = drawLandmarks(inp, out, pts_out*(opt.inputRes/opt.outputRes))
    image.save('./test/crop_landmark.jpg',im_show)
    ]]--
    ...
    if set == 'train' then
        ...
        if torch.uniform() < 0.5 then
           ...
           preds = getPreds(out:view(1,unpack(out:size():totable())))
	   pts_out = torch.reshape(preds, dataset.nJoints, 2)
           --[[
           image.save('./test/crop_flip.jpg',inp)
           preds = getPreds(out:view(1,unpack(out:size():totable())))
	   preds = torch.reshape(preds, dataset.nJoints, 2)
           im_show = drawLandmarks(inp, out, preds*(opt.inputRes/opt.outputRes))
           image.save('./test/crop_flip_landmark.jpg',im_show)
           matio.save('./test/crop_flip.mat',preds:float())
           ]]--
        end
    ...
    for i = 1,dataset.nJoints do
	pts_out[i] = pts_out[i]:div(opt.outputRes)
    end

    pts_out = torch.reshape(pts_out,dataset.nJoints*2)
    return inp,pts_out
end
