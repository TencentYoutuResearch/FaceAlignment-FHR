...
function transform_float(pt, center, scale, rot, res, invert)
...
-- Draw gaussian
-------------------------------------------------------------------------------
function drawGaussian_float(img, pt, sigma)
    -- Draw a 2D gaussian
    -- Check that any part of the gaussian is in-bounds
    local tmpSize = math.ceil(3*sigma)
    local ul = {math.floor(pt[1] - tmpSize + 0.5), math.floor(pt[2] - tmpSize + 0.5)} 
    local br = {math.floor(pt[1] + tmpSize + 0.5), math.floor(pt[2] + tmpSize + 0.5)}
    -- If not, return the image as is
    if (ul[1] > img:size(2) or ul[2] > img:size(1) or br[1] < 1 or br[2] < 1) then return img end
    -- Generate gaussian on float pts
    -- x0 = pt[1]; y0 = pt[2] 
    x0 = pt[1]
    y0 = pt[2]
    --[[
    for i=1,img:size(2) do -- x
        for j=1,img:size(1) do -- y
            img[j][i] = math.exp(-1 * (math.pow(i-x0,2) + math.pow(j-y0,2)) / (2 * math.pow(3*sigma,2)))
        end
    end 
    ]]--
    -- optimized version, limiting the area that has meaningful values, since most pixels of the image are zero
    for i=math.max(ul[1] - 3*sigma,1), math.min(br[1] + 3*sigma, img:size(2)) do -- x
        for j=math.max(ul[2] - 3*sigma,1), math.min(br[2] + 3*sigma, img:size(1)) do -- y 
            img[j][i] = math.exp(-1 * (math.pow(i-x0,2) + math.pow(j-y0,2)) / (2 * math.pow(3*sigma,2)))
        end
    end
    --[[
    print(x0,y0)
    print(img:sub(math.max(ul[2] - 3*sigma,1), math.min(br[2] + 3*sigma, img:size(1)), math.max(ul[1] - 3*sigma,1), math.min(br[1] + 3*sigma, img:size(2))))
    local matio = require "matio"
    matio.save('./test/img_hm_float.mat',img:float()) 
    img_hm = img:view(1,1,unpack(img:size():totable()))
    img_pts = getPreds_float(img_hm,sigma) 
    img_pts = torch.reshape(img_pts, 1, 2)
    print(img_pts)
    print(img_pts[1][1], img_pts[1][2])
    os.exit()
    ]]--
    return img
end
...
function drawGaussian(img, pt, sigma)
    ...
    --[[
    print(size)
    print(g:sub(g_y[1], g_y[2], g_x[1], g_x[2]))
    ...
    print(x0,y0)
    local matio = require "matio"
    matio.save('./test/img_hm.mat',img:float()) 
    ]]--
    return img
end
...
function drawLandmarks_pts(input, coords)
    local im = input:clone()
    local partColor = {1,1,1,2,2,2,0,0,0,0,3,3,3,4,4,4}
...
function drawLandmarks(input, hms, coords)
    local im = input:clone()
    --print(#hms)
...
function compileImages(imgs, nrows, ncols, res)
    ...
    for i = 1,#imgs do
        ...
        --print((c-1)*res+1)
        --print(c*res)
        --print(#totalImg:sub(1,3,(r-1)*res+1,r*res,(c-1)*res+1,c*res))
        --print(#imgs[i])
