...
function step(tag)
    ...
    for i,sample in loader[set]:run() do
	    ...
	    if tag == 'train' then
            ...
            im_show = drawLandmarks(input_, output_, preds*(256/opt.outputRes))
	        --image.save('./results/out_' .. indices[1] .. '.jpg', im_show)
	        --matio.save('./results/out_' .. indices[1] .. '.mat',preds:float())
	        --os.exit()
